...
int main()
{
    ...
    // 创建map容器
    std::map<int, std::unique_ptr<MyMonster>> monsterData;
    // 创建monster并分配编号
    ...
    {
        std::cout << "Find monster: " << it->second->getName() << std::endl;
    }
    else
    {
    ...
    for(int i = 0; i < codeList.size(); ++i)
    {
        ...
        if(it != monsterData.end())
        {
            it->second->beAttacked(hpVec[i]);
        }
    }
    // 输出剩余血量
    std::for_each(monsterData.begin(), monsterData.end(), [](std::pair<const int, std::unique_ptr<MyMonster>>& tmp){
    ...
    // 统计低血量monster数量
    int count = std::count_if(monsterData.begin(), monsterData.end(), [=](std::pair<const int, std::unique_ptr<MyMonster>>& tmp){
